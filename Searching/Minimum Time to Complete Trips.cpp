...
public:
    long long minimumTime(vector<int>& time, int totalTrips) {
        long long low = 1, 
        high = (long long) *max_element(time.begin(), time.end()) * (long long) totalTrips, 
        ans = -1;
        
        while (low <= high) {
            long long mid = low + (high - low) / 2;
            long long trips = getNumberOfTrips(time, mid);
    ...
    long long getNumberOfTrips(vector<int>& times, long long time) {
        long long ans = 0;
        for (auto x : times) {
            ans = ans + (time / (long long) x);
        }
        return ans;
    }
};
