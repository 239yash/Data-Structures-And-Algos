    ...
    int countPaths(int n, vector<vector<int>>& roads) {
        ...
        while (!pq.empty()) {
            ...
            for (auto iter : adj[node]) {
                ...
                if (time + newtime < dist[adjnode]) {
                    dist[adjnode] = time + newtime;
                    pq.push({time + newtime, adjnode});
                    countPaths[adjnode] = countPaths[node] % mod;
                }
                else if (time + newtime == dist[adjnode]) {
                    countPaths[adjnode] = (countPaths[adjnode] + countPaths[node]) % mod;  
                }
            }
        }
